...
# --- Visualization ---
ablation_df.set_index("Exp")[["Precision", "Recall", "F1-score"]].plot(kind="bar", figsize=(10, 6))
plt.title("Ablation Experiment: Performance Comparison")
plt.ylabel("Score")
plt.ylim(0, 1)
...
# Compare Modern_Video+Modern_Audio vs Traditional_Video+Traditional_Audio
scores_a = ablation_df[ablation_df["Exp"] == "Modern_Video + Modern_Audio"]["F1-score"].values
scores_b = ablation_df[ablation_df["Exp"] == "Traditional_Video + Traditional_Audio"]["F1-score"].values
if len(scores_a) > 0 and len(scores_b) > 0:
    t_stat, p_value = ttest_rel(scores_a, scores_b)
    print(f"\nT-test between Modern and Traditional (F1-score): t = {t_stat:.4f}, p = {p_value:.4f}")
else:
