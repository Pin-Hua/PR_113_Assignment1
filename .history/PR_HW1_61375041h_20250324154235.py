...
def evaluate_highlights(predicted_summary, ground_truth_path):
    with open(ground_truth_path, 'r') as f:
        ...
        ground_truth_summary = np.array(ground_truth_data["user_summary"]).mean(axis=0) >= 0.7
...
def visualize_pred_gt(predicted, ground_truth_path, output_path="comparison.png"):
    with open(ground_truth_path, 'r') as f:
        ground_truth_data = json.load(f)
        ground_truth = np.array(ground_truth_data["user_summary"]).mean(axis=0) >= 0.7
...
def get_summary(video_path, summary_length):
    ...
    motion_scores = extract_video_features()
    energy, spectral_contrast = extract_audio_features(motion_scores)
        
    # Ensure all arrays are of the same length before summation
    min_length = min(len(motion_scores), len(energy), len(spectral_contrast))
    motion_scores = motion_scores[:min_length]
    ...
    top_segments = np.argsort(combined_scores)[-int((summary_length * fps)):]
    top_segments.sort()

    video_clip = mp.VideoFileClip(video_path)
    selected_clips = [video_clip.subclip(max(0, i / fps), min(summary_length, (i + 1) / fps)) for i in top_segments]
    final_clip = mp.concatenate_videoclips(selected_clips, method="compose")

    output_path = video_path.replace(".mp4", f"_summary_{summary_length}s.mp4")
...
def main():
    ...
    if args.summary_30s:
        summary_30s = get_summary(args.video, 30)
        print(f"Generated Summaries:\n30s: {summary_30s}")
    if args.summary_60s:
        summary_1min = get_summary(args.video, 60)
        print(f"Generated Summaries:\n1min: {summary_1min}")
