...
class HighlightDetector:
    ...
    def extract_audio_features(self, motion_scores):
        ...
        # Ensure audio features match the number of video frames exactly
        target_length = len(motion_scores)  # Match motion_scores length
        energy = np.interp(np.linspace(0, len(energy) - 1, target_length), np.arange(len(energy)), energy)
        spectral_variance = np.interp(np.linspace(0, len(spectral_variance) - 1, target_length), np.arange(len(spectral_variance)), spectral_variance)
    ...
    def generate_summary(self, duration=30):
        ...
        energy, spectral_contrast = self.extract_audio_features(motion_scores)
